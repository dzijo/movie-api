...
window.onload = function () {
    ...
    var load_button = <HTMLElement>document.getElementById("button-load");
    ...
    var rouletteButton = <HTMLElement>document.getElementById("button-roulette");
    if (rouletteButton !== null) {
        rouletteButton.addEventListener("click", toggleRouletteDialog);
    }

    var rollButton = <HTMLElement>document.getElementById("button-roll");
    if (rollButton !== null) {
...
function makeMovie(data: any) {
    let imagePath = baseImageUrl + data["poster_path"],
        id = data["id"],
...
function toggleRouletteDialog() {
    let dialog = <HTMLElement>document.getElementById("genre-dialog-box");
    if (dialog !== null) {
...
function roulette() {
    ...
    for (let i = 0; i < form.length; i++) {
        let button = form[i] as HTMLInputElement;
        if (button.checked) {
...
function loadMovies(genre: number) {
    var baseUrl;
    var list_container = <HTMLElement>document.getElementById("list-container");
...
function openRandomMovie(genre: number) {
...
function saveRouletteGenre(genre: string) {
